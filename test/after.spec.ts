...
describe('Validator::after(field, date)', () => {
  ...
  it('should fail when date input is not valid', async () => {
    const v = await Validator.make({
      data: { at: '2020-05-10' },
      rules: { at: 'after:invalid date string' }
    })
    expect(v.fails()).toBe(true)
    expect(v.getErrors()).toHaveProperty('at')
  })

  it('should succeed when date input is after specified min date', async () => {
    const v = await Validator.make({
      data: { at: '2020-05-15' },
      rules: { at: 'after:2020-05-01' }
    })
    expect(v.fails()).toBe(false)
    expect(v.getErrors()).not.toHaveProperty('at')
  })
  ...
  describe('today as argument', () => {
    it('should succeed when date input is after specified max date', async () => {
      const v = await Validator.make({
